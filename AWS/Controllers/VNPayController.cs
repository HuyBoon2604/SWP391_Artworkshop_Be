...
namespace AWS.Controllers
{
    ...
    public class VNPayController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetOrderPre(string OrderPremiumIDLog)
        {
            try
            {
                var check = await this.context.OrderPremiumLogs.Where(x => x.OrderPremiumLogId.Equals(OrderPremiumIDLog)).FirstOrDefaultAsync();
                if (check != null)
                {
                    string ip = "256.256.256.1";
                    string url = _configuration["VnPay:Url"];
                    string returnUrl = _configuration["VnPay:ReturnPremiumPath"];
                    string tmnCode = _configuration["VnPay:TmnCode"];
                    string hashSecret = _configuration["VnPay:HashSecret"];
                    VnPayLibrary pay = new VnPayLibrary();
        ...
        public async Task<IActionResult> GetOrderCustome(string PaymentOrderCustomeID)
        {
            try
            {
                ...
                if (check != null)
                {
                    string ip = "256.256.256.1";
                    string url = _configuration["VnPay:Url"];
                    string returnUrl = _configuration["VnPay:ReturnCustomePath"];
                    string tmnCode = _configuration["VnPay:TmnCode"];
                    string hashSecret = _configuration["VnPay:HashSecret"];
                    VnPayLibrary pay = new VnPayLibrary();
